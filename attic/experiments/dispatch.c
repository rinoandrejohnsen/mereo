...
void _start() {
    ...
    if (count == 0) goto case_0;
    if (count == 1) goto case_1;
    goto case_2;
switch_end:
...
error_1:
    syscall3(1, 2, (long)"dispatch: 1: read input: ", 25);
    _write_value(count);
    _status = 1;
...
error_2:
    syscall3(1, 2, (long)"dispatch: 2: write terminal \"nothing\\n\": ", 41);
...
error_3:
    syscall3(1, 2, (long)"dispatch: 3: write terminal \"one byte\\n\": ", 42);
    _write_value(written);
    _status = 1;
...
error_4:
    syscall3(1, 2, (long)"dispatch: 4: write terminal \"many bytes\\n\": ", 44);
    _write_value(written);
    _status = 1;
