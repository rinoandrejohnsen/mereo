...
void _start() {
    ...
    source_descriptor = syscall3(2, (long)"lorem_ipsum.txt", 0, 0);
    if (__builtin_expect(!(source_descriptor >= 0), 0)) goto error_1_source;

    sink_descriptor = syscall3(2, (long)"copy_out.txt", 577, 420);
    if (__builtin_expect(!(sink_descriptor >= 0), 0)) goto error_2_sink;

    count = syscall3(0, source_descriptor, (long)buffer, capacity);
    if (__builtin_expect(!(count >= 0), 0)) goto error_3_read_source;

    written = syscall3(1, sink_descriptor, (long)buffer, count);
    if (__builtin_expect(!(written == count), 0)) goto error_4_write_sink;

release_sink:
    syscall1(3, sink_descriptor);
release_source:
...
error_1_source:
    ...
    syscall3(1, 2, (long)"catview: 1: source \"lorem_ipsum.txt\": ", 38);
...
error_2_sink:
    if (sink_descriptor == -4 || sink_descriptor == -32) goto release_source;   /* -EINTR/EPIPE: graceful shutdown */
    syscall3(1, 2, (long)"catview: 2: sink \"copy_out.txt\": ", 33);
    _write_value(sink_descriptor);
    _status = 1;
    __asm__("# stage 2" : "+r"(_status));
    goto release_source;

error_3_read_source:
    if (count == -4 || count == -32) goto release_sink;   /* -EINTR/EPIPE: graceful shutdown */
    syscall3(1, 2, (long)"catview: 3: read source: ", 25);
    _write_value(count);
    _status = 1;
...
error_4_write_sink:
    if (written == -4 || written == -32) goto release_sink;   /* -EINTR/EPIPE: graceful shutdown */
    syscall3(1, 2, (long)"catview: 4: write sink: ", 24);
