...
void _start() {
    ...
    file_descriptor = syscall3(2, (long)"missing.txt", 0, 0);
    if (__builtin_expect(!(file_descriptor >= 0), 0)) goto attempt_1_2;
resume_1:

    count = syscall3(0, file_descriptor, (long)buffer, capacity);
    if (__builtin_expect(!(count >= 0), 0)) goto error_2_read_file;
...
attempt_1_2:
    file_descriptor = syscall3(2, (long)"lorem_ipsum.txt", 0, 0);
    if (__builtin_expect(!(file_descriptor >= 0), 0)) goto error_1_file;
    __asm__("# attempt_1_2" : "+r"(file_descriptor));
    goto resume_1;

error_1_file:
    if (file_descriptor == -4 || file_descriptor == -32) goto exit;   /* -EINTR/EPIPE: graceful shutdown */
    syscall3(1, 2, (long)"fallback: 1: file \"missing.txt\" \"lorem_ipsum.txt\": ", 51);
...
error_2_read_file:
    if (count == -4 || count == -32) goto release_file;   /* -EINTR/EPIPE: graceful shutdown */
    syscall3(1, 2, (long)"fallback: 2: read file: ", 24);
    _write_value(count);
    _status = 1;
...
error_3_write_terminal:
    ...
    syscall3(1, 2, (long)"fallback: 3: write terminal: ", 29);
