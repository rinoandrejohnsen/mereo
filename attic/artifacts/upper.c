...
scan:
    ...
    *(unsigned char *)(((long)buffer + i)) = (unsigned char)(c);

    i = (i + 1);

    if (i < count) goto scan;

    written = syscall3(1, output_descriptor, (long)buffer, count);
    if (__builtin_expect(!(written == count), 0)) goto error_2_write_output;
...
error_1_read_input:
    ...
    syscall3(1, 2, (long)"upper: 1: read input: ", 22);
    _write_value(count);
    _status = 1;
...
error_2_write_output:
    ...
    syscall3(1, 2, (long)"upper: 2: write output: ", 24);
