...
/* Find the next '\n' word-at-a-time. The scan is the only loop that touches
   every byte of the input, so it is the only one worth widening: XOR the word
   against a broadcast newline and the byte that was equal becomes zero, which
   the classic has-a-zero-byte test finds without a branch per byte. */
static inline u32 find_byte(const u8 *p, u32 n, u8 b) {
    const u64 ONES = 0x0101010101010101UL, HIGH = 0x8080808080808080UL;
    const u64 NL   = (u64)b * ONES;
    u32 i = 0;
    while (i + 8 <= n) {
        u64 w;
