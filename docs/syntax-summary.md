...
slots is 8192                  -- a CONSTANT, at the left margin
...
See [Buffers](memory.md),
[Scalars](memory.md), [Storage](memory.md).
...
scope whose body ends by repeating. See [Scopes](control-flow.md),
...
`likely` means what it means in C. See [Choosing a path](control-flow.md).
...
`x == 2` is a test; `x is 2` sets `x`. See [`ensure`](errors.md).
...
See [Choosing a value](control-flow.md).
...
See [Defining a resource](resources.md), [`open`](library.md).
...
See [Files and directories](library.md).
...
[Container](memory.md), [Span and builder](memory.md).
