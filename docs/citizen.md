...
non-zero exit has exactly one meaning. See [Error handling](errors.md).
...
[`builder`](memory.md) and writing it once is the program's job, not the
runtime's.
...
- [Error handling](errors.md) — `ensure`, and the record
- [Control flow](control-flow.md) — `leave program`, and where the tower is entered
