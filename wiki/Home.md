...
**[Design](Design)** — the one commitment everything follows from, where the
name comes from, and what was deliberately left out.

The language itself, in reading order:

- **[Syntax and semantics](Syntax)** — a complete program, and the shape of
  the surface.
- **[Control flow](Control-flow)** — scopes, and the two jumps everything
  else is made of.
- **[Memory and views](Memory)** — backings, typed accesses, layouts, spans.
- **[Templates](Templates)** — reuse by splicing, and what that forbids.
...
**[Standard library](Library)** — the two files, and why the split is where
it is.
...
- **[Safety](Safety)** — the barrier, what is removed, what is refused, what
  is still unchecked, and how far the analysis reaches.
- **[The exam](Exam)** — one program written twice, in hand-optimised C and
...
- **[Examples](Examples)** — complete programs, all of them compiled by the
  documentation build.
