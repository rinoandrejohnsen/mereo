Every program here is in the repository and compiled by the docs build, so none
can drift.
...
Two views doing the work: a `span` names the block being read and is narrowed
in place, a `builder` fills the output and checks that each append fits. No
offset arithmetic appears at any call site.
...
record layer including its own field arithmetic. The coreutils-shaped examples
— `ls`, `stat`, `basename`, `head`, `wc -l`, `getenv` — are checked against the
system's own versions where a comparison is meaningful.